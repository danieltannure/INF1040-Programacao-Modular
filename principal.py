...
def show_tela_login():
    ...
    botao_login = tk.Button(root, text="Login", command=lambda: get_credenciais(entrada_usuario.get(), entrada_senha.get()))
...
def show_tela_cadastro():
    ...
    botao_criar_conta = tk.Button(root, text="Criar Conta", command=lambda: send_add_conta(entrada_novo_usuario.get(), entrada_nova_senha.get(), bairro_var.get(),entrada_horario_inicio.get(), entrada_horario_fim.get()))
    botao_criar_conta.grid(row=4, column=1, columnspan=1, pady=10)

    botao_voltar_login = tk.Button(root, text="Voltar ao Login", command=show_tela_login)
...
def show_tela_principal_admin():
    ...
    botao_formacoes = tk.Button(root, height=5, text="Gerir Formações", command=show_tela_formacoes_admin)
    botao_formacoes.grid(row=1, rowspan=2, column=0, padx=10, pady=10)

    botao_filiais = tk.Button(root,height=5, text="Gerir Filiais", command=show_tela_filiais_admin)
    botao_filiais.grid(row=1, rowspan=2, column=1,padx=10, pady=10)

    botao_formacao = tk.Button(root,height=5, text="Gerir Cursos", command=show_tela_cursos_admin)
...
def show_tela_cursos_admin():
    ...
    cursos = ["(Selecione o Curso)", "Psicologia", "Programação", "Física", "Medicina", "História"]
    ...
    botao_turmas = tk.Button(root, text="Deletar", command= lambda: send_del_curso(cursos_var.get()))
...
def show_tela_cria_curso():
    ...
    botao_voltar = tk.Button(root, text="Adicionar Requisito", command= lambda: send_append_requisito(cursos_var.get()))
    ...
    botao_voltar = tk.Button(root, text="Criar Curso", command= lambda: send_add_curso(entrada_nome.get(),entrada_duracao.get(),entrada_duracao_aula.get(),lista_cursos_prequisito))
...
def send_add_curso(nome,duracao,cargahoraria,prereqs):
    try: 
        ...
        try:
            ...
            messagebox.showerror("Erro", "Por favor, insira um número inteiro válido nas duracao.")
            show_tela_cria_curso()
    except ValueError:
        messagebox.showerror("Erro", "Por favor, insira um número inteiro válido nas duracao.")
        show_tela_cria_curso()
...
def show_tela_criterio_admin():
    ...
    botao_deletar = tk.Button(root, text="Adicionar Avaliação ao Critério", command= lambda: send_add_avaliacao_ao_criterio(cursos_var.get(),avaliacoes_var.get()))
    botao_deletar.grid(row=3,column=1,padx=10,pady=10)

    botao_deletar = tk.Button(root, text="Adicionar Avaliação Nova ao Critério", command= lambda: show_nova_avaliacao_ao_criterio(cursos_var.get()))
...
def send_add_avaliacao_ao_criterio(id_curso,id_avaliacao):
    if id_curso == "(Selecione o Curso)":
        ...
        #baseado na conclusão da ação, se o status for OK, retornamos voltamos a tela de inicio normal
        #caso não, chamaremos futuramente a mensagem de erro do módulo de Erro coerente com o numero de erro retornado
        messagebox.showinfo("Concluido", f"O critério do curso {id_curso}, foi atualizado.")
        show_tela_criterio_admin()
...
def show_nova_avaliacao_ao_criterio(id_curso):
    if id_curso == "(Selecione o Curso)":
        ...
        botao_criar = tk.Button(root, text="Criar ", command= lambda: send_novo_add_avaliacao_ao_criterio(entrada_nome.get(),entrada_tipo.get(),id_curso))
...
def send_novo_add_avaliacao_ao_criterio(nome,tipo,curso):
    try:
        ...
        messagebox.showinfo("Avaliação Incluida", f"Agora avaliação {nome}, faz parte do critério do curso {curso}.")
    except ValueError:
...
def show_tela_levantamentos_admin():
    ...
    botao_formacoes = tk.Button(root, height=5, text="Turmas Lecionando", command=send_turmas_lecionando)
    botao_formacoes.grid(row=1, rowspan=2, column=0, padx=10, pady=10)

    botao_filiais = tk.Button(root,height=5, text="Equipe de Professores", command=show_equipe_professores)
    botao_filiais.grid(row=1, rowspan=2, column=1,padx=10, pady=10)

    botao_formacao = tk.Button(root,height=5, text="Média Avaliação", command=show_media_avaliacao)
    botao_formacao.grid(row=1, rowspan=2, column=2,padx=10, pady=10)

    botao_criterio = tk.Button(root,height=5, text="Turmas por Filiais", command=show_turma_por_filiais)
    botao_criterio.grid(row=1, rowspan=2, column=3,padx=10, pady=10)

    botao_criterio = tk.Button(root,height=5, text="Formações do Ano", command=lambda: send_formacoes_do_ano)
    botao_criterio.grid(row=1, rowspan=2, column=4,padx=10, pady=10)

    botao_criterio = tk.Button(root,height=5, text="Calendário", command=show_tela_calendario_admin)
    botao_criterio.grid(row=1, rowspan=2, column=5,padx=10, pady=10)

    botao_criterio = tk.Button(root,height=5, text="Cursos Ativos", command=send_cursos_ativos)
...
def show_tela_calendario_admin():
    ...
    combobox_filial.bind("<<ComboboxSelected>>", lambda event: send_atualizar_calendario())
...
def send_atualizar_calendario():
    ...
    for col, dia in enumerate(dias):
        tk.Label(frame_calendario, text=dia, borderwidth=1, relief="solid", width=15, height=2).grid(row=0, column=col)
        ...
        for col, dia in enumerate(week):
            if dia == 0:
                ...
                for turma in turmas_do_mes:
                    data_fim = turma["data_ini"] + timedelta(weeks=turma["duracao_semanas"])
                    if turma["filial"] == filtro_filial and turma["data_ini"] <= data <= data_fim:
                        if (data - turma["data_ini"]).days % 7 == 0:
                            cell_text += f"Turma {turma['id']}\n"
            cell = tk.Text(frame_calendario, width=15, height=8, borderwidth=1, relief="solid")  # Altura ajustada
...
def send_obter_turmas():
    ...
    return [
        ...
        {"id": 2, "is_online": True, "data_ini": datetime(2024, 6, 5), "duracao_semanas": 6, "horario": {"ini": 14, "fim": 16}, "filial": 2},
        {"id": 3, "is_online": False, "data_ini": datetime(2024, 6, 8), "duracao_semanas": 3, "horario": {"ini": 10, "fim": 12}, "filial": 1},
        {"id": 4, "is_online": True, "data_ini": datetime(2024, 6, 10), "duracao_semanas": 2, "horario": {"ini": 15, "fim": 17}, "filial": 2},
        {"id": 5, "is_online": False, "data_ini": datetime(2024, 6, 12), "duracao_semanas": 5, "horario": {"ini": 8, "fim": 10}, "filial": 3},
        {"id": 6, "is_online": True, "data_ini": datetime(2024, 6, 15), "duracao_semanas": 7, "horario": {"ini": 13, "fim": 15}, "filial": 1},
        {"id": 7, "is_online": False, "data_ini": datetime(2024, 6, 18), "duracao_semanas": 4, "horario": {"ini": 11, "fim": 13}, "filial": 2}
    ]
...
def send_formacoes_do_ano():
    ...
    dicionario_formaturas=[{"id_formatura":1,"id_aluno":9,"cursos_concluidos":[3,4]},{"id_formatura":2,"id_aluno":4,"cursos_concluidos":[3]},{"id_formatura":1,"id_aluno":3,"cursos_concluidos":[3,4]}]
    dicionario_formacao=[{"id":1,"nome":"Fisiculturismo Avançado","cursos":[3,4,5]},{"id":2,"nome":"Fisiculturismo Intermediário","cursos":[3,4]}]
    turmas_cursos=[{"id_curso":3,"id_turma":50},{"id_curso":4,"id_turma":51},{"id_curso":3,"id_turma":59},{"id_curso":5,"id_turma":40}]
    alunos_turmas=[{"id_turma":40,"id_aluno":3},{"id_turma":51,"id_aluno":4},{"id_turma":30,"id_aluno":3},{"id_turma":14,"id_aluno":9}]
    dicionario_turmas=[{"id":40,"is_online":False,"max_alunos":10,"data_ini":datetime(2024,6,14),"duracao_semanas":3,"horario":{"ini":4,"fim":5}},
                       {"id":51,"is_online":False,"max_alunos":10,"data_ini":datetime(2024,10,14),"duracao_semanas":10,"horario":{"ini":2,"fim":6}},
                       {"id":14,"is_online":True,"max_alunos":None,"data_ini":datetime(2024,1,2),"duracao_semanas":30,"horario":{"ini":1,"fim":2}},
                       {"id":30,"is_online":False,"max_alunos":10,"data_ini":datetime(2024,4,9),"duracao_semanas":7,"horario":{"ini":3,"fim":4}}]
...
def send_turmas_lecionando():
    ...
    #retorne esse valor 
    messagebox.showinfo("Turmas Lecionando","Neste momento, possuem-se x turmas lecionando.")
    show_tela_levantamentos_admin()
...
def show_equipe_professores():
    ...
    professores=[{"id":1,"nome":'A',"cursos":[1,2],"filiais":[4,2],"horarios":{"ini":5,"fim":8}},{"id":2,"nome":'B',"cursos":[5,2],"filiais":[9,2],"horarios":{"ini":9,"fim":10}}]
    indice_fileira= 1
...
def show_media_avaliacao():
    ...
    botao_ver_media = tk.Button(root, text="Ver Média", command=lambda: send_media(avaliacoes_var.get()))
...
def send_media(id_avaliacao):
    if id_avaliacao == "(Selecione uma Avaliação)":
        ...
        respostas=[{"notas":5},{"notas":6},{"notas":7},{"notas":4},{"notas":6}]
        notas=[]
...
def show_tela_formacoes_admin():
    ...
    formacoes = ["(Selecione uma Formação)", "Engenheiro de Software", "Análise de Dados", "Paisagismo", "Pesquisa Laboratorial", "Pintura em Aquarela"]
    ...
    botao_deletar = tk.Button(root, text="Deletar", command= lambda: send_del_formacao(formacoes_var.get()))
    botao_deletar.grid(row=3,column=1,padx=10,pady=10)
...
def show_tela_cria_formacao():
    ...
    botao_criar = tk.Button(root, text="Adicionar", command= lambda: send_append_curso(cursos_var.get()))
    botao_criar.grid(row=3, column=1,padx=10, pady=10)

    botao_voltar = tk.Button(root, text="Voltar", command=show_tela_formacoes_admin)
    botao_voltar.grid(row=4,column=0,padx=10,pady=10)

    botao_voltar = tk.Button(root, text="Criar Formação", command= lambda: send_add_formacao(entrada_nome.get(),lista_cursos_formacao))
...
def send_append_curso(id_curso):
    if id_curso == "(Selecione o Curso)":
        ...
        messagebox.showinfo("Curso Adicionado", f"O curso {id_curso} foi adicionado a formação.")
        show_tela_cria_formacao()
...
def show_tela_principal_aluno():
    ...
    botao_matricula = tk.Button(root, height=5, text="Matricular-se", command=show_tela_matricula_aluno)
    botao_matricula.grid(row=1, rowspan=2, column=0, padx=10, pady=10)

    botao_cursos = tk.Button(root,height=5, text="Turmas", command=show_tela_turmas_aluno)
    botao_cursos.grid(row=1, rowspan=2, column=1,padx=10, pady=10)

    botao_certificado = tk.Button(root,height=5, text="Cancelar Turma", command=show_tela_cancelamento_aluno)
...
def show_tela_matricula_aluno():
    ...
    botao_matricula = tk.Button(root, text="Matricular", command= lambda: send_add_matricula(aluno_da_sessao, cursos_var.get(),formato_aula_var.get()))
...
def send_add_matricula(id_aluno,id_curso,quer_online):
    ...
    if id_curso == "(Selecione o Curso)":
        ...
        messagebox.showinfo("Informação Inválida", "Você não selecionou Formato de Aula.")
        show_tela_matricula_aluno()
    else:
...
def show_tela_turmas_aluno():
    ...
    botao_faltas= tk.Button(root,height=5, text="Ver Faltas", command= lambda: send_get_faltas(turmas_var.get(),aluno_da_sessao))
    botao_faltas.grid(row=2, column=0, padx=10,pady=10)

    botao_situacao= tk.Button(root,height=5, text="Ver Situação Acadêmica", command= lambda: send_situacao(turmas_var.get(),aluno_da_sessao))
    botao_situacao.grid(row=2, column=1, padx=10,pady=10)

    botao_avaliacoes= tk.Button(root,height=5, text="Ver Avaliações", command= lambda: show_tela_avaliacoes_aluno(turmas_var.get()))
...
def show_tela_avaliacoes_aluno(id_turma):
    if id_turma == "(Selecione uma Turma)":
        ...
        botao_notas= tk.Button(root,height=5, text="Fazer Prova", command= lambda: show_tela_fazer_prova(avaliacoes_var.get(),aluno_da_sessao,id_turma))  #precisa do atributo turma para poder voltar a pagina que estava
        botao_notas.grid(row=2, column=0, padx=10,pady=10)

        botao_notas= tk.Button(root,height=5, text="Ver Nota", command= lambda: send_get_resposta_by_aluno(avaliacoes_var.get(),aluno_da_sessao,id_turma))  #precisa do atributo turma para poder voltar a pagina que estava
        botao_notas.grid(row=2, column=1, padx=10,pady=10)

        botao_gabarito= tk.Button(root,height=5, text="Ver Gabarito", command= lambda: show_tela_gabarito_aluno(avaliacoes_var.get(),aluno_da_sessao,id_turma)) #precisa do atributo turma para poder voltar a pagina que estava
...
def show_tela_fazer_prova(id_avaliacao,id_aluno,id_turma):
    if id_avaliacao == "(Selecione uma Avaliação)":
        ...
        respostas=[]
        perguntas=["Qual meu nome?\n 1.Daniel 2.Danie 3.Dani 4.Dan 5.Da","Qual meu sobrenome?\n 1.Tannure 2.Tannur 3.Tannu 4.Tann 5.Tan"]
        ...
        for i,pergunta in enumerate(perguntas):
            ...
            escala = tk.Scale(root, from_=1, to=5, orient=tk.HORIZONTAL, variable=numero_escolhido)
            escala.grid(row=3*i+2, column=0,columnspan=2, padx=10, pady=10)
            botao_resposta= tk.Button(root, text="Confirmar Resposta", command= lambda: respostas.append(numero_escolhido.get()))  
        ...
        botao_concluir= tk.Button(root,text="Finalizar Prova",command=lambda: send_add_respostas(id_aluno,id_avaliacao,respostas,id_turma)) #precisa do atributo turma para poder voltar a pagina que estava
...
def show_tela_gabarito_aluno(id_avaliacao,id_aluno,id_turma):
    if id_avaliacao == "(Selecione uma Avaliação)":
        ...
        botao_voltar = tk.Button(root, text="Voltar", command=lambda: show_tela_avaliacoes_aluno(id_turma))
...
def show_tela_cancelamento_aluno():
    ...
    botao_cancelar = tk.Button(root, text="Cancelar Matrícula", command=lambda: send_del_matricula(turmas_var.get(),aluno_da_sessao))
...
def show_tela_formacoes_aluno():
    ...
    botao_inscricao= tk.Button(root,height=5, text="Inscrever-se", command= lambda: send_add_formatura(formacoes_var.get(),aluno_da_sessao)) 
    botao_inscricao.grid(row=2, column=1, padx=10,pady=10)

    botao_certificado= tk.Button(root,height=5, text="Gerar Certificado", command= lambda: send_gerar_certificado(formacoes_var.get(),aluno_da_sessao)) 
...
def show_tela_principal_professor():
    ...
    botao_matricula = tk.Button(root, height=5, text="Lecionar Turmas", command=show_tela_opcoes_professor)
    botao_matricula.grid(row=1, rowspan=2, column=0, padx=10, pady=10)
...
def show_tela_opcoes_professor():
    ...
    matriz_aulas= [["id_turma1","id_curso1","Tijuca",{"ini":1,"fim":3}],
    ...
    for i, aula in enumerate(matriz_aulas):
        ...
        botao = tk.Button(root, text=texto_botao, command=lambda t=turma: send_add_leciona(t,professor_da_sessao))
        botao.grid(row=i+1, column=0, padx=10, pady=10)


    botao_cancelar = tk.Button(root, text="Cancelar", command=show_tela_principal_professor)
...
def show_tela_turmas_professor():
    ...
    botao_presenca = tk.Button(root,height=5, text="Adicionar Presença", command= lambda: show_tela_presenca_professor(turmas_var.get()))
    botao_presenca.grid(row=2, rowspan=2, column=0,padx=10, pady=10)

    botao_avaliacoes = tk.Button(root,height=5, text="Montar Avaliações", command= lambda:  show_tela_montagem_professor(turmas_var.get()))
...
def show_tela_presenca_professor(id_turma):
    ...
    botao_marcar = tk.Button(root, text="Marcar Falta", command=lambda: send_set_faltas(alunos_var.get(),id_turma))
...
def show_tela_montagem_professor(id_turma):
    ...
    if id_turma == "(Selecione uma Turma)":
        ...
        botao_montar = tk.Button(root, text="Montar", command=lambda: show_tela_montar_professor(avaliacoes_var.get(),id_turma,professor_da_sessao))
        botao_montar.grid(row=2,column=1,padx=10,pady=10)

def show_tela_montar_professor(id_avaliacao,id_turma,id_prof): #rever os parametros necessários definidos na set_avaliacao
    for widget in root.winfo_children():
        widget.destroy()
    ...
    botao_adicionar = tk.Button(root, text="Adicionar Pergunta", command=lambda: send_pergunta(entrada_enunciado,entrada_gabarito))
    botao_adicionar.grid(row=6,column=2,padx=10,pady=10)

    botao_voltar = tk.Button(root, text="Voltar", command=lambda: show_tela_montagem_professor(id_turma))
    botao_voltar.grid(row=7,column=0,padx=10,pady=10)

    botao_concluir=tk.Button(root,text="Concluir Montagem",command=lambda:send_set_avaliacao(id_avaliacao,entrada_nome.get(),entrada_tipo.get(),id_turma,id_prof,lista_gabarito_montagem,lista_perguntas_montagem))
...
def send_set_avaliacao(id_avaliacao,nome,tipo,id_turma,id_prof,gabarito,perguntas):
    try: 
        valor = int(tipo)
